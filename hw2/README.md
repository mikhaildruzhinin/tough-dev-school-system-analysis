...
Смущает, что получился довольно сильный coupling между системой исполнения заказов, системой ставок и разработкой гипотез для улучшения бизнеса, но пока не могу предложить исправление.
...
UPD:
Добавил схему компонентов, можно посмотреть по [ссылке](components/components.drawio.png).

Выбрал асинхронное взаимодействие между микросервисами с помощью брокера сообщений.
У каждого из микросервисов будет своя база данных, для хранения фотографий от исполнителей используется object storage хранилище.
Поскольку есть угроза ddos атак, доступ к приложению со стороны клиентов будет осуществляться через обратный прокси сервер.
Также предполагается добавить дополнительный микросервис API Gateway "перед" микросервисами, которые содержат бизнес логику, для унификации и упрощения взаимодействия с микросервисами со стороны клиентских приложений.
